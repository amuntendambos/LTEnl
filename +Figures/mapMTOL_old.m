...
if(strcmpi(FigType,'paper'))
    ...
    for i=3:max(topo_wgs(:,1))-1
        ...
        hline2=plot(topo_wgs(II,2),topo_wgs(II,3),'Color',[0.75 0.75 0.75]);
        if i==1
    ...
    xlabel('Longitude'); title('Prospect fields'); % ylabel('Latitude');
    %colorbar(); %ylabel(h, 'Red-Light Magnitude (M_L)');
    colormap(gca,R_colormap('red-light')); caxis(ML_c);
    xlim(XL); ylim(YL);
    %
    ...
    xlabel('Longitude'); title('Gas Storage Sites'); %ylabel('Latitude');
    %colorbar(); %ylabel(h, 'Red-Light Magnitude (M_L)');
    colormap(gca,R_colormap('red-light')); caxis(ML_c);
    xlim(XL); ylim(YL);
